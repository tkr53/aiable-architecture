...
このリポジトリは Claude Code のプラグインマーケットプレイスである。マーケットプレイスを追加して
プラグインをインストールする:
...
これで `aiable-architecture` スキル（文脈で自動起動）と、下記の 4 つの `/aiable-*` コマンドが入る。

## この Skill の使い方

`skills/aiable-architecture/SKILL.ja.md` がアーキテクチャの規律本体。
`skills/aiable-architecture/references/` に書式と変換テンプレートを置く。

- `skills/aiable-architecture/references/acceptance-format.ja.md` — acceptance.md の書式
- `skills/aiable-architecture/references/property-types.ja.md` — プロパティ 5 類型の rapid への変換
- `skills/aiable-architecture/references/incident-format.ja.md` — incident の書式と回帰テスト結線

コマンド入口（Claude Code で `/コマンド名 <capability>` として、または自律起動で）:
