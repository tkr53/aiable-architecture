...
- property: 登録成功後、平文 password（生バイト列）が復号後の保存値（salt+digest）に出現しない。保存値は検査前に hex 復号する。hex 文字列との比較は hex が 0-9a-f しか使わないため偽陽性になる（incident INC-002 参照）
- property: 同じ password を 2 回登録すると、保存されたハッシュは互いに異なる
- note: 極端に短い password の偶然一致は平文保存の失敗ではない。非出現の検査は 6 バイト以上の password に適用し、上のハッシュ相違の property が全長をカバーする
